 ...
 * 
 * 
 * Author: andip71, TheSSJ & sayeed99, 10.12.2014
 * 
 * Version 1.0.1
 *
...
#define DRIVER_AUTHOR "andip71 (Lord Boeffla), TheSSJ, sayeed99"
#define DRIVER_DESCRIPTION "Sweep2sleep"
		...
		if((touch_y >= y_boundary) && (touch_y <= (y_boundary + BLACK_AREA)))
		{
...

			{
				if(touch_x >= 70 && touch_x <= 225)
					key_press = KEY_BACK;
				if(touch_x >= 270 && touch_x <= 445)
					key_press = KEY_HOME;
				if(touch_x >= 505 && touch_x <= 661)
					key_press = KEY_MENU;
			}
